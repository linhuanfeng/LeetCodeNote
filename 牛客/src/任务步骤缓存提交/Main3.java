...
/*
{"stepId":1,"type":1,"action":"commit"}
{"stepId":2,"type":2,"action":"commit"}
{"stepId":3,"type":2,"action":"cancel"}
{"stepId":4,"type":1,"action":"cancel"}
{"stepId":5,"type":4,"action":"commit"}
{"stepId":6,"type":1,"action":"commit"}
...
public class Main3 {
    ...
    private static Step convertToStep(String stepStr) {
        ...
        step.setAction(actionStr[1].substring(1,actionStr[1].length()-1));
        return step;
    }
    ...
    public static long[] commitTask(List<Step> steps) {
        ...
        for (Step step : steps) {
            if(que.isEmpty()){
                ...
                if(!que.getLast().action.equals(step.action)){
                    que.removeLast(); // 反向就可以移除
                }else {
                    que.addLast(step);
                }
            }
